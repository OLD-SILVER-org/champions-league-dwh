...
class PlayerTransformer(BaseTransformer):
    ...
    def get_extracted_data(self, season) -> pd.DataFrame:
        ...
        path = os.path.join(self.SAVE_PATH, self.PLAYERS_LOCATION, str(season))
    ...
    def standardize_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "squad" in df.columns:
            df["squad"] = df["squad"].astype(str).str.split(n=1).str[-1].str.title()
    ...
    def add_keys(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """Player is a dim data, have nk at Extract process -> dont need more"""
        return df
