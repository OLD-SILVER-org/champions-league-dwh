...
class SquadScraper(SeleniumScraper):
    ...
    def scrape_season_data(self):
        """Scrape squad data for current season."""
    ...
    def scrape_data(self, season):
        ...
        try:
            ...
            squad_tbody = self.find_element(
                By.ID, "stats_squads_standard_for"
            ).find_element(By.TAG_NAME, "tbody")
            if not squad_tbody:
                return None
            self.logger.info(f"✅ Successfully fetched squad for season {season}")
            # Save the DataFrame
            ...
            self.logger.debug(f"❌ Scraping failed for season {season}: {e}")
            return None
        finally:
            self.quit()
    ...
    def save_data(self, dataframe, season):
        ...
        folder_path = os.path.join(self.SAVE_PATH, self.SQUADS_LOCATION, str(season))
        os.makedirs(folder_path, exist_ok=True)
        data_name = os.path.join(
            folder_path, f"{now.strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        )
        dataframe.to_csv(data_name, index=False)
        self.logger.info(f"✅ Data squads saved: {data_name}")
        pass
    ...
    def extract_squad_data(self, squad_tbody, season):
        ...
        for row in rows:
            try:
                ...
                # et team name and country details
                ...
                number_of_player = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="players_used"]'
                ).text
                matches_played = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="games"]'
                ).text
                ...
                self.logger.debug(f"❌ Error processing row {row.text}: {e}")
