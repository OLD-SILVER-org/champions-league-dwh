...
class MatchScraper(SeleniumScraper):
    ...
    def save_data(self, dataframe, season):
        ...
        folder_path = os.path.join(self.SAVE_PATH, self.MATCHS_LOCATION, str(season))
        os.makedirs(folder_path, exist_ok=True)
        data_name = os.path.join(
            folder_path, f"{now.strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        )
        dataframe.to_csv(data_name, index=False)
        self.logger.info(f"✅ Data saved: {data_name}")

    def scrape_data(self, season):
        """Scrape match data for a given season using Selenium."""
        url = self.get_season_link(season)
        self.logger.info(f"📌 Scraping season {season} link: {url}")

        try:
            self.get(url)  # Open URL using SeleniumScraper
            self.close_cookie_banner()
            # Find the match data table
            match_tbody = self.find_element(By.CLASS_NAME, "stats_table").find_element(
                By.TAG_NAME, "tbody"
            )

            if not match_tbody:
                self.logger.info(f"❌ No match data found for season {season}")
                return None

            self.logger.info(f"✅ Successfully fetched page for season {season}")
        ...
        for row in rows:
            try:
                ...
                week = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="gameweek"]'
                ).text
                day = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="dayofweek"]'
                ).text
                date = row.find_element(By.CSS_SELECTOR, 'td[data-stat="date"]').text
                time = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="start_time"]'
                ).text
                ...
                xG_Home_element = row.find_elements(
                    By.CSS_SELECTOR, 'td[data-stat="home_xg"]'
                )
                xG_Home = xG_Home_element[0].text if xG_Home_element else ""

                xG_Away_element = row.find_elements(
                    By.CSS_SELECTOR, 'td[data-stat="away_xg"]'
                )
                ...
                venue = row.find_element(By.CSS_SELECTOR, 'td[data-stat="venue"]').text
                referee = row.find_element(
                    By.CSS_SELECTOR, 'td[data-stat="referee"]'
                ).text
                ...
                if match_td and match_td[0].find_elements(By.TAG_NAME, "a"):
                    match_href = (
                        match_td[0].find_element(By.TAG_NAME, "a").get_attribute("href")
                    )
                    match_report = re.search(r"/matches/([a-zA-Z0-9]+)/", match_href)
                    match_report = match_report.group(1) if match_report else ""
                # Append to list
                df_list.append(
                    [
                        season,
